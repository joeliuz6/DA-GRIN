...
class GraphFiller(Filler):
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_target_dominant = batch['target_dominant']
        ...
        ### target_dominant_data operation #### 

        # Unpack batch
        batch_data_td, batch_preprocessing_td = self._unpack_batch(batch_target_dominant)

        # Compute masks
        mask_td = batch_data_td['mask'].clone().detach()
        batch_data_td['mask'] = torch.bernoulli(mask_td.clone().detach().float() * self.keep_prob).byte()
        eval_mask_td = batch_data_td.pop('eval_mask', None)
        eval_mask_td = (mask_td | eval_mask_td) - batch_data_td['mask']  # all unseen data

        y_td = batch_data_td.pop('y')

        # Compute predictions and compute loss
        res_td = self.predict_batch(batch_target_dominant, preprocess=False, postprocess=False)
        imputation_td, predictions_td = (res_td[0], res_td[1]) if isinstance(res_td, (list, tuple)) else (res_td, [])
        fwd_repr_td,bwd_repr_td,imputation_repr_td = res_td[2],res_td[3],res_td[4]
        td_repr = torch.cat([fwd_repr_td,bwd_repr_td],dim=1)

        # trim to imputation horizon len
        imputation_td, mask_td, eval_mask_td, y_td = self.trim_seq(imputation_td, mask_td, eval_mask_td, y_td)
        predictions_td = self.trim_seq(*predictions_td)

        if self.scaled_target:
            target_td = self._preprocess(y_td, batch_preprocessing_td)
        else:
            target_td = y_td
            imputation_td = self._postprocess(imputation_td, batch_preprocessing_td)
            for i, _ in enumerate(predictions_td):
                predictions_td[i] = self._postprocess(predictions_td[i], batch_preprocessing_td)

        #loss_td = self.loss_fn(imputation_td, target_td, mask_td)
        loss_td = 0
        for pred_td,pred_t in zip(predictions_td,predictions_t):
            loss_td += self.tradeoff * self.loss_fn(pred_td, pred_t, mask_td)
        ...
        cl_loss = Coral(imputation_repr_t,imputation_repr_td)
