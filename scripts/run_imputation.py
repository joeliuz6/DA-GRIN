...
def get_dataset(dataset_name):
    if dataset_name[:3] == 'air':
        ...
        target_dominant_dataset = datasets.MissingValuesTarget_dominant(p_fault=0., p_noise=0.25)
    else:
        raise ValueError(f"Dataset {dataset_name} not available in this setting.")
    return dataset,target_dataset,target_dominant_dataset
...
def run_experiment(args):
    ...
    dataset,target_dataset,target_dominant_dataset = get_dataset(args.dataset_name)
    ...
    torch_target_dominant_dataset = dataset_cls(*target_dominant_dataset.numpy(return_idx=True),
                                mask=target_dataset.training_mask,
                                eval_mask=target_dataset.eval_mask,
                                window=args.window,
                                stride=args.stride)
    ...
    dm = SpatioTemporalDataModule(torch_dataset,torch_target_dataset,torch_target_dominant_dataset, train_idxs=train_idxs, val_idxs=val_idxs, test_idxs=test_idxs,
                                  **data_conf)
    dm.setup()
