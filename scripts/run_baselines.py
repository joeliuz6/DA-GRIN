...
class MatrixFactorizationImputer(Imputer):
    ...
    def __init__(self, rank=10, loss='mae', verbose=0):
        ...
        method = MatrixFactorization(rank=rank, learning_rate=0.02,max_iters=2,verbose=verbose)
...
def get_dataset(dataset_name):
    ...
    elif dataset_name == 'discharge_noise':
...
def run(imputer, dataset, train_slice):
    ...
    data_full = pd.read_csv(r'C:\Users\89457\Desktop\optimizaiton\Spatial-Temporal\spatial-temporal\grin-main\datasets/discharge\SSC_discharge.csv',index_col=0)
    # Evaluate model
    y_true = dataset.numpy()[test_slice]
    #print(y_true.shape)
    eval_mask = dataset.eval_mask[test_slice]
    stdd = np.array(data_full.std()).reshape(1,20)
    meann = np.array(data_full.mean()).reshape(1,20)
    for metric, metric_fn in metrics.items():
        error = metric_fn(y_hat, y_true, eval_mask)
        print(f'{imputer.name} on {ds_name} {metric}: {error:.4f}')

        print('real value space')

        error = metric_fn((y_hat-1) * stdd + meann, (y_true-1) * stdd+meann, eval_mask)
        print(f'{imputer.name} on {ds_name} {metric}: {error:.4f}')
